...
class LargestBSTSubtree {
    ...
    public Answer largestBSTSubtreeInner(TreeNode node) {
        ...
        Answer left = largestBSTSubtreeInner(node.left);
        Answer right = largestBSTSubtreeInner(node.right);

        if (left.isValid && right.isValid) {
            if (
                (node.left == null || node.val > left.max) 
                && 
                (node.right == null || node.val < right.min)) 
                {
                return new Answer(true, left.size + right.size + 1, node.left == null ? node.val : left.min,
                                  node.right == null ? node.val : right.max);
                }
        }
